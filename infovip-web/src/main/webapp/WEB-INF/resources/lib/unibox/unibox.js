var UniBox = function () {
    ...
    function highlightSearchWords(string, searchString) {
        ...
        var words = searchString.replace(/[^a-zA-Z0-9íűáéőúóüöäöüÍÁÉŰÚŐÓÜÖÄÖÜß]|\s+|\r?\n|\r/gmi, " ").replace(/[^a-zA-Z0-9íűáéőúóüöäöüÍÁÉŰÚŐÓÜÖÄÖÜß]/g, " ").split(' ');
    ...
    function updateSuggestBox(data) {
        ...
        jQuery.each(suggestOrderToUse, function (idx, key) {
            ...
            jQuery.each(values, function (index, suggest) {
                ...
                if (suggest['link'] != undefined && suggest['link'] != '') {
                    ...
                    suggestLine += '<span class="uniboxSearchContent" data="'+ Base64.encode(JSON.stringify(suggest)) +'">' + highlightSearchWords(suggest['name'], searchStringXss) + '</span>';
                }
        ...
        jQuery(selectables).mousedown(function () {
            ...
            if (enterCallbackResult != undefined) {
                	try {
                		enterCallbackResult.call(this, q, href, jQuery.parseJSON( Base64.decode( jQuery(this).find('.uniboxSearchContent:first').attr('data') ) ) );
                	} catch(ex) {
                		console.log(ex);
                	}
            }
        ...
        if (event.keyCode == 13) {
            ...
            if (enterCallbackResult != undefined) {
                ...
                if (selectedEntryIndex != -1) {
                    ...
                    if (enterCallbackResult != undefined) {
                        try {
                            enterCallbackResult.call(this, selectedText, href, jQuery.parseJSON( Base64.decode( searchBoxParent.find('.unibox-selectable.active .uniboxSearchContent:first').attr('data') ) ) );
                        } catch (ex) {
                            console.log(ex);
                        }
                    }
